...
def call_argparse():
    ...
    parser.add_argument('--agent', type=str, default='claude', choices=['gpt-4-turbo', 'gpt-4o', 'claude'], help='agent name')
    parser.add_argument('--task', type=str, default='bias_discovery', help='task to solve, default is bias discovery')
    ...
    parser.add_argument('--classifiers', nargs='+', type=int, default=None, help='synthetic classifier numbers (used with data.json mode)')
    parser.add_argument('--labels', nargs='+', type=str, default=None, help='labels (used with direct label mode, e.g., clip)')
            ...
            while current_r < 10:
                ...
                try:
                    ...
                    if current_r == 0:
                        ...
                        if args.task == 'clip':
                            prev_pos_avg, prev_neg_avg = main_utils.load_prev_context(round_num=current_r-1, setting='context_clip')
                        else:
                        ...
                        if first_resp is None:
                            logger.error(f"{log_id}: ask_agent returned None for initial response in Round {current_r}. Aborting rounds for this classifier.")
                    ...
                    for i in range(20): # Max 20 interactions
                        ...
                        if resp is None:
                            logger.error(f"{log_id}: ask_agent returned None during interaction {i} in Round {current_r}. Aborting rounds for this classifier.")
                            ask_agent_failed_non_badrequest = True
                            break

                        tools.update_experiment_log(role='agent', type="text", type_content=str(resp))
                        tools.generate_html(n_path2save, name=f'experiment_{args.n_experiment}_r{current_r}')

                        if "[BIAS LABEL]" in resp:
                            logger.info(f"{log_id}: Bias found in Round {current_r}, interaction {i}. Ending round early.")
                            break
                        ...
                        try:
                            ...
                            logger.error(f"{log_id}: Experiment execution failed in Round {current_r}, interaction {i}: {exec_e}", exc_info=True)
                    ...
                    for attempt in range(1, 6):
                        try:
                            ...
                            if pos_avg is not None and neg_avg is not None:
                                ...
                                break
                            logger.warning(f"{log_id}: Save attempt {attempt} for Round {current_r} returned None values, retrying...")
                        except Exception as save_e:
                            logger.error(f"{log_id}: Save results attempt {attempt} for Round {current_r} failed: {save_e}", exc_info=True)
                        time.sleep(2)

                    if not save_successful:
                        logger.error(f"{log_id}: All save attempts failed for Round {current_r}. Aborting rounds for this classifier.")
                        break
                    ...
                    if args.task == 'clip':
                        logger.info(f"{log_id}: Round {current_r} completed. Stats: pos_avg={pos_avg}, neg_avg={neg_avg}")
                    else:
                        logger.info(f"{log_id}: Round {current_r} completed. Stats: pos_avg={pos_avg}, neg_avg={neg_avg}, exemplar_avg={main_utils.avg_acts}")

                    logger.info(f"{log_id}: Round {current_r} duration: {time.time() - round_start_time:.2f}s")

                    current_r += 1

                except anthropic.BadRequestError as bad_req_e:
                    logger.error(f"{log_id}: Anthropic BadRequestError in Round {current_r}: {bad_req_e}. Retrying this round after a delay.")
                    badrequest_count += 1
                    if badrequest_count > 3:
                        logger.error(f"{log_id}: Anthropic BadRequestError in Round {current_r}: {bad_req_e}. Aborting rounds for this classifier.")
                        break
                    time.sleep(10)

                except Exception as general_e:
                    logger.error(f"{log_id}: Unexpected error during Round {current_r} processing: {general_e}", exc_info=True)
                    break
