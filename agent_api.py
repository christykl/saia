...
from utils.api_utils import format_api_content, image2str, is_base64, str2image
...
from utils.YOLOClassifier import YOLOClassifier
...
class Tools:
    ...
    def __init__(
        self,
        device: str,
        SyntheticExemplars: SyntheticExemplars = None,
        ...
        self.text2image_model = self._load_text2image_model(
            model_name=text2image_model_name
        )
        self.p2p_model_name = p2p_model_name
        if self.p2p_model_name == 'ip2p':
            self.p2p_model = self._load_pix2pix_model(
                model_name=self.p2p_model_name
            )  # consider maybe adding options for other models like pix2pix zero
        elif self.p2p_model_name == 'instdiff':
            self.p2p_model = InstructDiffusion(
                batch_size=1,
                config_path='utils/InstructDiffusion/configs/instruct_diffusion.yaml',
                model_path='utils/InstructDiffusion/checkpoints/v1-5-pruned-emaonly-adaption-task.ckpt',
                device=self.device,
            )
        self.experiment_log = []
        self.im_size = 224
    ...
    def edit_images(
        ...
        for i in range(len(base_images)):
            if self.p2p_model_name == 'instdiff':
                ...
                edited_imgs_obj = self.p2p_model(
                    [editing_prompts[i]], [base_imgs_obj[i]]
                )
            else:
    ...
    def _load_pix2pix_model(self, model_name):
        ...
        if model_name == 'ip2p':  # instruction tuned pix2pix model
            print('Loading InstructPix2Pix model from timbrooks/instruct-pix2pix...')
            device = self.device
    ...
    def _load_text2image_model(self, model_name):
        ...
        if model_name == 'sd':
            print(
                'Loading Stable Diffusion 3.5 Medium model from stabilityai/stable-diffusion-3.5-medium...'
            )
            device = self.device
            model_id = 'stabilityai/stable-diffusion-3.5-medium'
            sdpipe = StableDiffusion3Pipeline.from_pretrained(
            ...
            print('Loading SDXL Turbo model from stabilityai/sdxl-turbo...')
            device = self.device
            ...
            print('Loading Flux Image Generator...')
