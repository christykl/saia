...
class MainUtils:
    def __init__(self, path2prompts, path2save, agent, obj, tools, system, experiment):
        ...
        # Initialize exemplars if available (self.tools.exemplars is None when not using exemplars)
        if self.tools.exemplars is not None:
            data_acts = [act for (act, image) in self.tools.dataset_exemplars(self.system)]
            self.avg_acts = mean(data_acts)
        else:
            self.avg_acts = 1.0  # Default normalization factor when exemplars not used
    ...
    def load_prev_context(self, round_num=0, setting="context"):
        ...
        for entry in history:
            if entry['round'] == round_num:
                ...
                # Display dataset exemplars only if available
                if self.tools.exemplars is not None:
                ...
                # Display average dataset exemplar score only if exemplars are available
                if self.tools.exemplars is not None:
